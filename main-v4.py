    ...
    def _move_thread(self, speed, acceleration):
        ...
        for step in range(steps_to_move):
            ...
            if step < accel_steps:
                ...
                current_speed = acceleration * (steps_to_move - step) / speed  # Decelerating
            else:
...
class StreamingHandler(server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            ...
            content = PAGE.format(GUI_WIDTH=GUI_WIDTH, GUI_HEIGHT=GUI_HEIGHT).encode('utf-8')
            ...
            self.end_headers()
            self.wfile.write(f'{{"position": {stepper.current_position}}}'.encode())
        else:
            self.send_error(404)
            self.end_headers()
    ...
    def do_POST(self):
        ...
        if self.path == '/api/move':
            ...
            stepper.move(steps, direction, speed, acceleration)
            ...
            Thread(target=take_burst, args=(start_pos, end_pos, num_images, speed, acceleration)).start()
...
def take_burst(start_pos, end_pos, num_images, speed, acceleration):
    ...
    stepper.move_to(start_pos, speed, acceleration)
    while stepper.is_moving:
        time.sleep(0.1)
        if stepper.emergency_stop:
            return
    ...
    for pos in positions:
        ...
        # Move to position
        stepper.move_to(pos, speed, acceleration)
...
stepper = StepperMotor(pi, DIR_PIN, STEP_PIN, SLEEP_PIN)
